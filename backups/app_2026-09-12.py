...
def ejecutar_paso_con_progreso(comando, caja_texto, contenedor_estado, mensaje_estado):
    """Ejecuta un subproceso emitiendo logs en tiempo real para verificar actividad."""
...
if st.session_state["clase_seleccionada"] and st.session_state["materia_seleccionada"]:
    ...
    if metodo_origen == "📁 Usar vídeo ya guardado en la carpeta 'clases/'":
        ...
        if archivo_cargado and materia_nombre and clase_nombre:
            directorio_final = os.path.join(CARPETA_BASE, materia_nombre.replace(" ", "_"), clase_nombre.replace(" ", "_"))
            os.makedirs(directorio_final, exist_ok=True)
            ruta_archivo_final = os.path.join(directorio_final, archivo_cargado.name)
    ...
    if st.button("🚀 Iniciar Procesamiento", type="primary"):
        if not ruta_archivo_final or not os.path.exists(ruta_archivo_final):
            ...
            try:
                # Paso 1: Transcripción (40% de la barra)
                barra_progreso.progress(10, text="Paso 1/3: Extrayendo audio y transcribiendo...")
                cmd_transcribir = [sys.executable, "-u", "transcribir.py", ruta_archivo_final, ruta_transcripcion]
                ejecutar_paso_con_progreso(
                    cmd_transcribir,
                    caja_consola,
                    estado_actual,
                    "Paso 1/3: Transcribiendo con Apple Silicon (MLX)..."
                )
                barra_progreso.progress(50, text="Paso 1/3 completado.")

                # Paso 2: Generación de Apuntes + Auditoría (Ollama)
                barra_progreso.progress(60, text="Paso 2/3: Sintetizando apuntes con Qwen 2.5...")
                cmd_apuntes = [sys.executable, "-u", "generar_apuntes.py", ruta_transcripcion, ruta_apuntes]
                ejecutar_paso_con_progreso(
                    cmd_apuntes,
                    caja_consola,
                    estado_actual,
                    "Paso 2/3: Generando apuntes técnicos y auditoría..."
                )
